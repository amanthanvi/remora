...
fn claim_follow_up_for_turn(reducer: &AppStoreReducer, key: &ThreadKey, text: &str, turn_id: &str) {
    let inputs = vec![upstream::UserInput::Text {
        text: text.to_string(),
        text_elements: Vec::new(),
    }];
    ...
    assert!(reducer.try_claim_first_queued_follow_up(key).is_some());
    reducer.bind_first_claimed_follow_up_to_turn(key, &inputs, turn_id);
}

fn authoritative_user_message(
    text: &str,
    turn_id: &str,
    item_id: &str,
) -> HydratedConversationItem {
...
#[test]
fn authoritative_completed_turn_consumes_its_claimed_queued_follow_up() {
    let reducer = AppStoreReducer::new();
    let key = ThreadKey {
        server_id: "srv".to_string(),
        thread_id: "thread".to_string(),
    ...
    reducer.upsert_thread_snapshot(ThreadSnapshot::from_info("srv", make_thread_info("thread")));
    claim_follow_up_for_turn(&reducer, &key, "repeat this", "turn-follow-up");

    let mut authoritative = ThreadSnapshot::from_info("srv", make_thread_info("thread"));
    authoritative.items.push(authoritative_user_message(
        "repeat this",
        "turn-follow-up",
        "server-user-item",
    ));
...
#[test]
fn authoritative_older_identical_message_does_not_consume_claimed_follow_up() {
    let reducer = AppStoreReducer::new();
    let key = ThreadKey {
        server_id: "srv".to_string(),
        thread_id: "thread".to_string(),
