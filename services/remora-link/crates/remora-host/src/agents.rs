...
use crate::config::{CodexTransport, HostConfig};
...
enum CodexMode {
    ...
    /// child per iroh stream. Automatic local transport, independent of the
    /// CLI's installation channel.
    UnixProxy,
...
pub struct AgentManager {
    ...
    codex_sessions: Arc<crate::codex_session::CodexSessions>,
    background_relay: Option<Arc<crate::background_relay::BackgroundRelay>>,
}
...
impl AgentManager {
    ...
    pub async fn new(config: Arc<ArcSwap<HostConfig>>) -> anyhow::Result<Self> {
        ...
        let codex_detection = if snapshot.agents.codex.enabled {
            detect_codex(
                &snapshot.agents.codex.bin,
                snapshot.agents.codex.transport,
                &daemon_env,
            )
            .await
        } else {
        ...
        let background_relay = snapshot
            .background_relay
            .clone()
            .map(|relay| {
                crate::background_relay::BackgroundRelay::load(
                    relay,
                    crate::paths::state_dir()?.join("background-relay-v1.json"),
        ...
        Ok(Self {
            ...
            codex_sessions: Arc::new(crate::codex_session::CodexSessions::default()),
            background_relay,
        })
    }
    ...
    pub(crate) fn background_relay(
        &self,
    ) -> Option<&Arc<crate::background_relay::BackgroundRelay>> {
        self.background_relay.as_ref()
    }
    ...
    pub async fn shutdown(&self) {
        self.codex_sessions.shutdown().await;
    ...
    pub async fn start_agent_with_session(
        ...
        match agent {
            "codex" => {
                let pool = Arc::clone(&self.codex_sessions);
                let manager = self.clone();
                let websocket = self.codex_mode != CodexMode::Stdio;
                let task = pool
                    .attach(
                        stream,
                        session,
                        last_seen,
                        websocket,
                        Arc::downgrade(&self.session_registry),
                        move |backend| async move { manager.start_codex(backend).await },
                    )
                    .await?;
                Ok(StartedAgentSession::new(
                    task,
                    "serving retained Codex session".to_owned(),
                ))
            }
            other => {
                let kind =
    ...
    async fn restart_codex(&self) -> anyhow::Result<()> {
        ...
        if self.codex_mode == CodexMode::Stdio {
            self.codex_sessions.shutdown().await;
            return Ok(());
        }
    ...
    async fn start_codex<S>(&self, iroh_stream: S) -> anyhow::Result<StartedAgentSession>
    where
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        match self.codex_mode {
    ...
    async fn start_codex_unix_proxy<S>(
        &self,
        mut iroh_stream: S,
    ) -> anyhow::Result<StartedAgentSession>
    where
    ...
    async fn start_codex_ws<S>(&self, mut iroh_stream: S) -> anyhow::Result<StartedAgentSession>
    where
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
    ...
    async fn ensure_codex_unix_running(&self) -> anyhow::Result<CodexUnixEndpoint> {
        ...
        let env = self.daemon_launch_env().await;
        #[cfg(unix)]
        if default_codex_control_socket_path(&env)
            .is_some_and(|path| !crate::paths::unix_socket_path_fits(&path))
        {
            let socket = crate::paths::control_socket_path()?.with_file_name("codex.sock");
            return self
                .ensure_codex_unix_running_with_endpoint(
                    CodexUnixEndpoint::custom_socket(bin, socket),
                    &env,
                )
                .await;
        }
    ...
    async fn start_codex_stdio<S>(&self, mut iroh_stream: S) -> anyhow::Result<StartedAgentSession>
    where
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        let bin = {
...
/// Probe the user-installed Codex CLI. Automatic selection prefers Unix proxy,
/// then TCP websocket, then stdio. The daemon command requires a separately
/// managed standalone installation, so advertising its help is not sufficient
/// evidence to select it automatically. Any failure
...
async fn detect_codex(
    ...
    for candidate in candidates {
        ...
        let daemon_supported = transport == CodexTransport::UnixDaemon
            && listen_supported
            && proxy_supported
            && codex_app_server_daemon_supported(&candidate, env).await;
        let mode = match transport {
            CodexTransport::UnixDaemon if daemon_supported => CodexMode::UnixDaemon,
            CodexTransport::UnixProxy if listen_supported && proxy_supported => {
                CodexMode::UnixProxy
            }
            CodexTransport::Websocket if listen_supported => CodexMode::Websocket,
            CodexTransport::Stdio => CodexMode::Stdio,
            CodexTransport::Auto if listen_supported && proxy_supported => CodexMode::UnixProxy,
            CodexTransport::Auto if listen_supported => CodexMode::Websocket,
            CodexTransport::Auto => CodexMode::Stdio,
            _ => continue,
        };
...
mod tests {
    #[cfg(unix)]
    #[tokio::test]
    async fn automatic_transport_does_not_assume_advertised_daemon_is_installed() {
        use std::os::unix::fs::PermissionsExt;

        let directory = tempfile::tempdir().unwrap();
        let bin = directory.path().join("codex");
        std::fs::write(&bin, "#!/bin/sh\nif [ \"$2\" = \"--help\" ]; then echo --listen; exit 0; fi\nif [ \"$3\" = \"--help\" ]; then exit 0; fi\nexit 1\n").unwrap();
        std::fs::set_permissions(&bin, std::fs::Permissions::from_mode(0o700)).unwrap();
        let environment = LaunchEnvironment::current();
        let detection =
            detect_codex(bin.to_str().unwrap(), CodexTransport::Auto, &environment).await;
        assert!(detection.available);
        assert_eq!(detection.mode, CodexMode::UnixProxy);
        let websocket = detect_codex(
            bin.to_str().unwrap(),
            CodexTransport::Websocket,
            &environment,
        )
        .await;
        assert!(websocket.available);
        assert_eq!(websocket.mode, CodexMode::Websocket);
    }
