...
use remora_bridge_core::Hydrator;
pub use remora_bridge_core::{
    IndexEntry as CoreIndexEntry, ListFilter, ListPage, ListSort, ThreadIndex as CoreThreadIndex,
};
use remora_codex_proto::{SessionSource, Thread, ThreadSourceKind, ThreadStatus};

pub const CLI_VERSION: &str = concat!("remora-amp-bridge/", env!("CARGO_PKG_VERSION"));
    ...
    async fn open_and_hydrate_skips_native_thread_when_amp_session_already_indexed() {
        ...
        index
            .insert(IndexEntry {
                ...
                name: Some("Existing Remora title".to_string()),
                preview: "Existing Remora preview".to_string(),
                created_at: 1,
                updated_at: 2,
                archived: false,
                forked_from_id: None,
        ...
        assert_eq!(page.data[0].name.as_deref(), Some("Existing Remora title"));
        assert_eq!(
