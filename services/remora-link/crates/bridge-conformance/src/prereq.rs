//! Per-target prerequisite probes run before spawning a backend. Missing
//! prerequisites return a reason; explicitly selected live tests treat that
//! reason as a failure, not successful conformance.
