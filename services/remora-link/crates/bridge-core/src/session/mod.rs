...
impl Session {
    ...
    pub fn enqueue(&self, mut payload: Value) -> u64 {
        let seq = {
            ...
            if payload.get("method").is_some() {
                self.state_revision.fetch_add(1, Ordering::SeqCst);
            }
            if let Some(id) = payload.pointer("/params/turn/id").and_then(Value::as_str) {
                match payload.get("method").and_then(Value::as_str) {
                    Some("turn/started") => {
                        self.active_turns.lock().unwrap().insert(id.to_owned());
                    }
                    Some("turn/completed") => {
                        self.active_turns.lock().unwrap().remove(id);
                    }
                    _ => {}
                }
            }
    ...
    /// Only runtime-originated notifications/requests affect repair barriers;
    /// read RPC responses must not invalidate the repair that requested them.
    pub fn state_barrier(&self) -> (u64, u64) {
        let _ring = self.ring.lock().unwrap();
        (self.instance, self.state_revision.load(Ordering::SeqCst))
    }

    pub fn invalidate_state_barrier(&self) {
        let _ring = self.ring.lock().unwrap();
        self.state_revision.fetch_add(1, Ordering::SeqCst);
    ...
    pub fn has_active_turns(&self) -> bool {
        !self.active_turns.lock().unwrap().is_empty()
    }
    ...
    /// An old drainer must not detach the connection that replaced it.
    pub fn drop_attachment_generation(&self, generation: u64) {
    ...
    pub fn current_attachment_generation(&self) -> u64 {
