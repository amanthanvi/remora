...
impl SessionRegistry {
    ...
    pub fn release_if_current(&self, session: &Arc<Session>) {
        let mut inner = self.inner.lock().unwrap();
    ...
    pub fn tick(&self, pending_grace: Duration, idle_ttl: Duration) {
        ...
        self.inner.lock().unwrap().retain(|_, session| {
            !session.detached_for(idle_ttl)
                || session.has_outstanding_requests()
                || session.has_active_turns()
        });
    }
}
...
mod tests {
    ...
    #[test]
    fn detached_active_turn_survives_idle_ttl_until_completion() {
        let reg = SessionRegistry::new(SessionRegistryConfig::default());
        let session = reg.get_or_create("device".into(), "codex");
        session.install_attachment(None);
        session.enqueue(
            serde_json::json!({"method":"turn/started","params":{"turn":{"id":"turn-1"}}}),
        );
        session.drop_attachment();
        reg.tick(Duration::ZERO, Duration::ZERO);
        assert!(reg.get("device", "codex").is_some());
        session.enqueue(
            serde_json::json!({"method":"turn/completed","params":{"turn":{"id":"turn-1"}}}),
        );
        reg.tick(Duration::ZERO, Duration::ZERO);
        assert!(reg.get("device", "codex").is_none());
    }
