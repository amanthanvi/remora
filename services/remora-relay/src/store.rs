...
impl RelayStore {
    ...
    pub fn create_installation(&self, now_ms: i64) -> Result<IssuedInstallation> {
...
fn migrate(connection: &Connection) -> Result<()> {
    ...
    let has_lease_id = {
        let mut statement = connection.prepare("PRAGMA table_info(push_outbox)")?;
        statement
            .query_map([], |row| row.get::<_, String>(1))?
            .collect::<std::result::Result<Vec<_>, _>>()?
            .into_iter()
            .any(|column| column == "lease_id")
    };
    if !has_lease_id {
