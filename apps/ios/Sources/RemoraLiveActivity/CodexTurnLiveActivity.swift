...
struct CodexTurnLiveActivity: Widget {
    ...
    private var warningColor: Color { RemoraPalette.warning.color(for: .dark) }
    private var dangerColor: Color { RemoraPalette.danger.color(for: .dark) }
    private var primaryText: Color { RemoraPalette.textPrimary.color(for: .dark) }
    private var secondaryText: Color { RemoraPalette.textSecondary.color(for: .dark) }
    private var mutedText: Color { RemoraPalette.textMuted.color(for: .dark) }
                ...
                DynamicIslandExpandedRegion(.center) {
                    VStack(alignment: .leading, spacing: 3) {
                        Text(context.attributes.prompt)
                            .font(.system(size: 11, weight: .medium, design: RemoraPalette.fontDesign))
                        ...
                        HStack(spacing: 6) {
                            ...
                            if context.state.toolCallCount > 0 {
                                Label("\(context.state.toolCallCount)", systemImage: "chevron.left.forwardslash.chevron.right")
                                    .font(.system(size: 9, design: RemoraPalette.fontDesign))
    ...
    private func remoraLogo(size: CGFloat) -> some View {
        Image("remora_logo")
            .resizable()
            .interpolation(.high)
            .scaledToFit()
    ...
    private func phaseBadge(_ state: CodexTurnAttributes.ContentState) -> some View {
        Text(phaseBadgeText(state))
            .font(.system(size: 10, weight: .medium, design: RemoraPalette.fontDesign))
    ...
    private func metaChip(systemImage: String, text: String) -> some View {
        HStack(spacing: 4) {
            ...
            Text(text)
                .font(.system(size: 10, design: RemoraPalette.fontDesign))
                .foregroundStyle(mutedText)
                .lineLimit(1)
        }
        .padding(.trailing, 10)
    ...
    private func ctxBadge(_ percent: Int) -> some View {
        Text("\(percent)%")
            .font(.system(size: 9, weight: .semibold, design: RemoraPalette.fontDesign))
    ...
    private func liveTimer(context: ActivityViewContext<CodexTurnAttributes>, size: CGFloat) -> some View {
        if isActive(context.state) {
            Text(timerInterval: context.attributes.startDate...Date.distantFuture, countsDown: false)
                .font(.system(size: size, design: RemoraPalette.fontDesign))
                .monospacedDigit()
...
private struct AdaptiveLiveTimer: View {
    ...
    private var timerColor: Color {
        let text = RemoraPalette.textSecondary.color(for: colorScheme)
    ...
    var body: some View {
        ...
        .font(.system(size: 15, design: RemoraPalette.fontDesign))
        .monospacedDigit()
        .fontWeight(.regular)
