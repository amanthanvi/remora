...
/// `RemoraWatchApp` so the singleton survives the whole app lifetime.
...
final class RemoraWatchAppDelegate: NSObject, WKApplicationDelegate {
    static let shared = RemoraWatchAppDelegate()
...
/// Root @main for the Remora Watch app. Vertically paginated TabView
/// makes the three hero surfaces reachable via crown/swipe.
@main
struct RemoraWatchApp: App {
    @WKApplicationDelegateAdaptor(RemoraWatchAppDelegate.self) private var appDelegate
    ...
    var body: some Scene {
        ...
        WKNotificationScene(
            controller: RemoraNotificationController.self,
            category: "remora.task.complete"
        )
...
final class RemoraNotificationController: WKUserNotificationHostingController<NotificationScreen> {
