...
/// Pure deep-link parser for `remora-watch://…` URLs. The watch root view
...
final class WatchDeepLinkRouter: ObservableObject {
    ...
    /// Called by `RemoraWatchApp.onOpenURL` AND by AppIntent `.perform`
    ...
    nonisolated static func destination(for url: URL) -> Destination? {
        guard url.scheme == "remora-watch" else { return nil }
        switch url.host {
        case "task":
