...
final class WatchSessionBridge: NSObject, WCSessionDelegate {
    ...
    /// `.remoraThreadPreferencesDidChange` and triggers a refreshed push.
    ...
    private func reloadComplicationsIfChanged(store: WatchAppStore) {
        ...
        center.reloadTimelines(ofKind: "RemoraCircularComplication")
        center.reloadTimelines(ofKind: "RemoraCornerComplication")
        center.reloadTimelines(ofKind: "RemoraRectangularComplication")
    }

    private nonisolated func handle(_ payload: [String: Any]) {
        guard
            let raw = payload["remora.snapshot"] as? Data,
