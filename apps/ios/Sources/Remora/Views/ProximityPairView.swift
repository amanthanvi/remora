...
struct ProximityPairView: View {
    ...
    private var backgroundGradient: some View {
        ...
        return LinearGradient(
            colors: [
                Color.black,
                RemoraTheme.surface.opacity(0.4 + 0.4 * s)
            ],
            startPoint: .top,
            endPoint: .bottom
        )
        ...
        return GeometryReader { geo in
            ...
            ZStack {
                ForEach(0..<3, id: \.self) { i in
                    ...
                    Circle()
                        .stroke(RemoraTheme.accent.opacity(Double(opacity)), lineWidth: 2)
                ...
                Circle()
                    .fill(
                        RadialGradient(
                            colors: [
                                RemoraTheme.accent.opacity(0.6 * Double(s)),
                                RemoraTheme.accent.opacity(0)
                            ],
                            center: .center,
                            startRadius: 0,
    ...
    private var centerStack: some View {
        ...
        return VStack(spacing: 18) {
            Image(systemName: peerIconName)
                ...
                .shadow(color: RemoraTheme.accent.opacity(Double(animatedScore) * 0.8), radius: 24)
    ...
    private func distanceReadout(frame: ProximityFrame) -> some View {
        HStack(alignment: .firstTextBaseline, spacing: 6) {
            Text(frame.distanceM.map { String(format: "%.1f", $0) } ?? "—")
                .remoraFont(size: 56, weight: .semibold)
                .foregroundColor(RemoraTheme.textPrimary)
            ...
            if let v = frame.velocityMS, abs(v) > 0.1 {
                ...
                let color = v > 0 ? RemoraTheme.accentStrong : RemoraTheme.textMuted
