...
/// Ghostty surface (via `RemoraGhosttyTerminal`); the renderer mediates the
...
final class GhosttyTerminalRenderer {
    ...
    private var terminal: RemoraGhosttyTerminal?
    ...
    func attach(to view: UIView) {
        ...
        attachedView = view
        do {
            let terminal = try RemoraGhosttyTerminal(view: view)
    ...
    private func applyConfigToRenderer(_ config: TerminalConfig, renderer: TerminalRenderer) {
        if !didSetConfigDir {
            ...
            if let dir = cachesDir?.appendingPathComponent("remora/terminal", isDirectory: true) {
...
final class RemoraGhosttyInputView: UIView, UIKeyInput, UITextInputTraits {
    ...
    var accessoryBar: RemoraTerminalAccessoryBar? {
        didSet {
...
final class RemoraTerminalAccessoryBar: UIView {
...
final class GhosttyHostView: UIView, UIGestureRecognizerDelegate, UIEditMenuInteractionDelegate {
    ...
    private let keyboardOverlay = RemoraGhosttyInputView()
    private let accessoryBar = RemoraTerminalAccessoryBar()
    ...
    private func presentEditMenu() {
        ...
        let configuration = UIEditMenuConfiguration(identifier: "remora.terminal.edit-menu" as NSString, sourcePoint: center)
        if let rect = anchor {
