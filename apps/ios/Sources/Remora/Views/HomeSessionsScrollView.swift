...
struct HomeSessionsScrollView: UIViewRepresentable {
    ...
    func updateUIView(_ view: HomeSessionsScrollUIView, context: Context) {
        ...
        view.apply(
            ...
            zoomLevel: zoomLevel,
            showMascotFooter: showMascotFooter,
            topInset: topInset,
...
final class HomeSessionsScrollUIView: UIView {
    ...
    private let mascotFooterHostingController = UIHostingController(rootView: AnyView(EmptyView()))
    ...
    private var mascotFooterCountEligible = false
    private var mascotFooterHostVisible = false
    private var mascotFooterEntranceStarted = false
    ...
    override init(frame: CGRect) {
        ...
        mascotFooterHostingController.view.backgroundColor = .clear
        mascotFooterHostingController.view.isHidden = true
        contentView.addSubview(mascotFooterHostingController.view)
    ...
    func apply(
        ...
        self.mascotFooterCountEligible = showMascotFooter && !sessions.isEmpty && sessions.count <= 10
    ...
    private func relayout(animated: Bool) {
        ...
        if shouldShowMascotFooter {
            let h = mascotFooterHeight(width: width)
        ...
        if animated {
            UIView.animate(withDuration: zoomSnapDuration, delay: 0, options: [.curveEaseOut]) {
                ...
                self.mascotFooterHostingController.view.frame = footerFrame
            ...
            for (container, frame) in frames { container.frame = frame }
            mascotFooterHostingController.view.frame = footerFrame
    ...
    private var shouldShowMascotFooter: Bool {
        mascotFooterCountEligible && zoomLevel == 1 && !isPinching
    }

    private func mascotFooterHeight(width: CGFloat) -> CGFloat {
        let videoWidth = min(max(0, width - 48), 340)
        return videoWidth * 9.0 / 16.0 + 32
    }

    private func refreshMascotFooterVisibility() {
        let visible = shouldShowMascotFooter
        guard mascotFooterHostVisible != visible else { return }
        mascotFooterHostVisible = visible
        if visible {
        ...
        mascotFooterHostingController.view.isHidden = !visible
    }
...
private struct HomeMascotFooterView: View {
...
final class HomeRowContainer: UIView {
    ...
    init(scrollHost: HomeSessionsScrollUIView) {
        ...
        pinchHighlight.backgroundColor = UIColor(RemoraTheme.accent).withAlphaComponent(0.14)
    ...
    override func didMoveToWindow() {
        ...
        guard window != nil,
              !RemoraPlatform.rendersAsMacApp,
    ...
    func setPinchBlurProgress(_ progress: CGFloat) {
        ...
        if RemoraPlatform.rendersAsMacApp { return }
    ...
    private func updateActionsVisuals() {
        ...
        if offsetX > 0 {
            actionsBackground.backgroundColor = UIColor(RemoraTheme.accent)
            ...
            actionsBackground.backgroundColor = UIColor(RemoraTheme.danger)
            actionsBackground.alpha = tintAlpha
