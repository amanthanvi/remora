                ...
                ForEach(files) { file in
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 8, style: .continuous)
                            .fill(RemoraTheme.surfaceLight.opacity(0.72))
                    )
                }
            }
        }
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(plugins, id: \.path) { plugin in
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 8, style: .continuous)
                            .fill(RemoraTheme.accent.opacity(0.12))
                    )
                }
            }
        }
...
struct ConversationComposerModeChip: View {
    ...
    private var background: Color {
        mode == .plan ? RemoraTheme.accent : RemoraTheme.surfaceLight
    }
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(RemoraTheme.codeBackground.opacity(0.92))
        )
    }
...
private struct ConversationComposerGoalRowView: View {
    ...
    private var progressTextTint: Color {
        guard let progress = budgetProgress else { return RemoraTheme.textSecondary }
        if progress >= 1.0 { return RemoraTheme.danger }
        if progress >= 0.85 { return RemoraTheme.warning }
        ...
        if #available(iOS 26.0, *) {
            ...
            content
                .background(
                    LinearGradient(
                        colors: [
                            RemoraTheme.codeBackground.opacity(0.92),
