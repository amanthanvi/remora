                        ...
                        ForEach(identifiedKeyValueEntries(entries)) { entry in
                            ...
                            HStack(alignment: .top, spacing: 8) {
                                Text(entry.value.key + ":")
                                    .remoraFont(size: contentFontSize, weight: .semibold)
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        ForEach(identifiedItems) { item in
                            ...
                            HStack(alignment: .top, spacing: 8) {
                                Circle()
                                    .fill(item.index == identifiedItems.count - 1 ? kindAccent : RemoraTheme.textMuted)
    ...
    private func inlineTextSection(id: String, label: String, content: String) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text(verbatim: visibleText(content, id: id))
                .remoraMonoFont(size: contentFontSize)
            ...
            ZStack {
                ...
                if let renderedImage {
                    ...
                    Text(loadError ?? "Image unavailable")
                        .remoraFont(.caption)
                        .foregroundColor(loadError == nil ? RemoraTheme.textSecondary : RemoraTheme.danger)
...
#Preview("Tool Call Card") {
    ZStack {
        ...
        ToolCallCardView(model: RemoraPreviewData.sampleToolCallModel)
            .padding(20)
    }
}
#endif
