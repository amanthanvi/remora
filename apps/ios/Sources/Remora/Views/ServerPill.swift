    ...
    var body: some View {
        ...
        .overlay(
            Capsule(style: .continuous)
                .stroke(
                    isSelected ? RemoraTheme.accent.opacity(0.75) : RemoraTheme.textMuted.opacity(0.25),
...
private struct AgentRuntimeOverflowBadge: View {
    ...
    var body: some View {
        Text("+\(count)")
            .remoraMonoFont(size: 9, weight: .bold)
            .foregroundStyle(RemoraTheme.textPrimary)
            .lineLimit(1)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 5, style: .continuous)
                    .stroke(RemoraTheme.textPrimary.opacity(0.28), lineWidth: 0.55)
            )
