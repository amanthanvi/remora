...
struct ConversationComposerModalCoordinator<Content: View>: View {
    ...
    var body: some View {
        content
            ...
            .sheet(isPresented: $showModelSelector) {
                ...
                .presentationBackground(RemoraTheme.surface)
            }
                ...
                VStack(alignment: .leading, spacing: 14) {
                    VStack(alignment: .leading, spacing: 12) {
                        HStack(alignment: .center) {
                            ...
                            Text(currentRuntimeSupportsPermissionOverrides ? (usesThreadDefaults ? "Using defaults" : "Custom override") : "Runtime managed")
                                ...
                                .background(
                                    Capsule()
                                        .fill((usesThreadDefaults ? RemoraTheme.surfaceLight : RemoraTheme.accentStrong).opacity(0.16))
                                )
                        }
                        ...
                        HStack(spacing: 10) {
                            ...
                            permissionSummaryTile(
                                ...
                                accent: hasAuthoritativeThreadPermissions ? RemoraTheme.textSecondary : RemoraTheme.warning
                            )
                        }
                    }
                    .padding(14)
                    .background(
                        RoundedRectangle(cornerRadius: 20, style: .continuous)
                            .fill(RemoraTheme.surface.opacity(0.82))
                    )
                    .overlay(
                        RoundedRectangle(cornerRadius: 20, style: .continuous)
                            .stroke(RemoraTheme.border.opacity(0.55), lineWidth: 1)
                    )
    ...
    private var unsupportedPermissionRuntimeCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("This agent does not support Remora-side thread permission overrides, so approval and sandbox choices are not sent for this session.")
                .foregroundStyle(RemoraTheme.textMuted)
                .remoraFont(.caption)
        }
        .padding(14)
    ...
    private func permissionSummaryTile(
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(RemoraTheme.surfaceLight.opacity(0.78))
        )
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .stroke(RemoraTheme.border.opacity(0.45), lineWidth: 1)
        )
    }
    ...
    private func permissionSection<SectionContent: View>(
        ...
        .background(
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .fill(RemoraTheme.surface.opacity(0.74))
        )
        .overlay(
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .stroke(RemoraTheme.border.opacity(0.5), lineWidth: 1)
        )
    }
        ...
        Menu {
            ...
            .background(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .fill(RemoraTheme.surfaceLight.opacity(0.9))
            )
            .overlay(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .stroke(RemoraTheme.border.opacity(0.45), lineWidth: 1)
