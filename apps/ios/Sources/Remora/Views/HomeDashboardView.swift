...
struct HomeDashboardView: View {
        ...
        ToolbarItem(placement: .principal) {
            if chrome == .sidebar {
                ...
                HStack(spacing: 4) {
                    ...
                    SupporterBadges(tierIndices: 2..<4)
                }
            }
        }
        if chrome == .full {
    ...
    private var dashboardBackground: some View {
        if RemoraPlatform.rendersAsMacApp && chrome == .sidebar {
            Color.clear
        } else {
    ...
    /// Mascot illustration shown on the empty home screen. Positioned in
    ...
    /// then crossfades to the looping APNG, matching the home footer.
    private var emptyHomeMascot: some View {
        GeometryReader { proxy in
            let h = proxy.size.height
            let w = proxy.size.width
            let mascotWidth = min(max(180, w * 0.55), 260)
            let mascotHeight = mascotWidth * 202.0 / 360.0
            EmptyHomeMascotView()
...
struct SessionCanvasLine: View {
    ...
    var body: some View {
        ...
        .background(alignment: .leading) {
            if isActive {
                RemoraTheme.accent.opacity(0.3).frame(width: 2)
            }
        }
        .background(isActive ? RemoraTheme.accent.opacity(0.02) : Color.clear)
    ...
    private var telemetryDashboard: some View {
        ...
        if hasAny {
            VStack(alignment: .leading, spacing: 0) {
                if let goal {
                    ...
                    if hasMetrics {
                        ...
                        Rectangle()
                            .fill(RemoraTheme.border.opacity(0.4))
                            .frame(height: 0.5)
                            .padding(.vertical, 8)
                    }
                }
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        HStack(alignment: .top, spacing: 14) {
                            statCell(icon: "chevron.left.forwardslash.chevron.right",
                                     ...
                                     valueColor: RemoraTheme.textPrimary,
                                     label: "files")
                            statCell(icon: nil,
                                     valuePrefix: nil,
                                     value: pct.map { "\($0)%" },
                                     valueColor: (pct ?? 0) > 80 ? RemoraTheme.warning : RemoraTheme.warning.opacity(0.85),
                                     label: "context")
                        }
                        HStack(alignment: .top, spacing: 14) {
                            statCell(icon: nil,
                                     value: adds > 0 ? "+\(adds.formatted(.number.grouping(.automatic)))" : nil,
                                     valueColor: RemoraTheme.accent,
                                     label: "added")
                        ...
                        HStack(alignment: .top, spacing: 14) {
                            statCell(icon: nil,
                                     ...
                                     valueColor: RemoraTheme.danger.opacity(0.85),
                                     label: "removed")
                            statCell(icon: "clock",
            ...
            .overlay(alignment: .top) {
                Rectangle().fill(RemoraTheme.border.opacity(0.5))
                    .frame(height: 0.5)
            }
            .overlay(alignment: .bottom) {
                Rectangle().fill(RemoraTheme.border.opacity(0.5))
    ...
    private func goalStatusTint(_ status: AppThreadGoalStatus) -> Color {
        switch status {
        case .active: return RemoraTheme.accent
        case .paused: return RemoraTheme.textMuted
        case .blocked, .usageLimited, .budgetLimited: return RemoraTheme.warning
        case .complete: return RemoraTheme.success
        }
    }
    ...
    private var userMessageLine: some View {
        ...
        if !message.isEmpty && message != title {
            ...
            FormattedText(text: message, lineLimit: zoomLevel >= 4 ? 3 : 1)
                ...
                .padding(.leading, 8)
                .padding(.trailing, 6)
                .background(RemoraTheme.accent.opacity(0.06))
                .overlay(alignment: .leading) {
                    RemoraTheme.accent.opacity(0.55).frame(width: 2)
                }
                .clipShape(
    ...
    private var cwdFooter: some View {
        HStack(spacing: 8) {
            ...
            if isActive {
                ...
                .overlay(
                    Capsule()
                        .stroke(RemoraTheme.accent.opacity(0.4), lineWidth: 0.5)
                )
                .clipShape(Capsule())
                .fixedSize()
            }
    ...
    private func toolRowView(_ entry: AppToolLogEntry) -> some View {
        ...
        .remoraFont(size: toolLogFontSize)
        .frame(maxWidth: .infinity, alignment: .leading)
    }
    ...
    private func toolIconView(for tool: String) -> some View {
        ...
        case "MCP":
            Image(systemName: "desktopcomputer")
                .remoraFont(size: toolLogFontSize - 1, weight: .semibold)
        case "Tool":
            Image(systemName: "wrench.and.screwdriver")
                .remoraFont(size: toolLogFontSize - 1, weight: .semibold)
        case "Bash":
            Text("$").remoraFont(size: toolLogFontSize - 1, weight: .semibold)
        case "Edit":
            Text("✎").remoraFont(size: toolLogFontSize - 1, weight: .semibold)
    ...
    private var responsePreview: some View {
        ...
        // naturally updates at ~8Hz instead of forcing `RemoraMarkdownView`
    ...
    private func siblingPill(member: ThreadLineageMember, isCurrent: Bool) -> some View {
        HStack(spacing: 5) {
            Circle()
                .fill(isCurrent ? RemoraTheme.accent : RemoraTheme.textMuted.opacity(0.5))
                .frame(width: 5, height: 5)
            Text(member.title)
                .lineLimit(1)
                .truncationMode(.tail)
        ...
        .background(
            Capsule()
                .fill(isCurrent ? RemoraTheme.accent.opacity(0.12) : RemoraTheme.surface.opacity(0.6))
        )
        .overlay(
            Capsule()
                .stroke(isCurrent ? RemoraTheme.accent.opacity(0.6) : RemoraTheme.border.opacity(0.6), lineWidth: 1)
        )
    }
}
...
/// message bodies (`RemoraFont.conversationBodyPointSize × textScale`) so
...
private struct MarkdownMatchedTitleFont: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .font(.custom(
                RemoraFont.markdownFontName,
                size: RemoraFont.conversationBodyPointSize * textScale
            ))
            .fontWeight(.medium)
    }
}
