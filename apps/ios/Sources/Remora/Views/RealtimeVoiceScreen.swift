...
struct RealtimeVoiceScreen: View {
    ...
    private var promptFillColor: Color {
        RemoraTheme.surface.opacity(colorScheme == .dark ? 0.82 : 0.92)
    }

    private var promptStrokeColor: Color {
        RemoraTheme.border.opacity(colorScheme == .dark ? 0.55 : 0.8)
    }

    private var controlFillColor: Color {
        RemoraTheme.surfaceLight.opacity(colorScheme == .dark ? 0.72 : 0.88)
    }
    ...
    private func transcriptLine(
        ...
        return Text(entry.text)
            .font(RemoraFont.styled(textStyle, weight: fontWeight))
