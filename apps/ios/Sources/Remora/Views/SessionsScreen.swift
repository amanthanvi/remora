...
struct SessionsScreen: View {
    ...
    private func screenLayout(derived: SessionsDerivedData) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if derived.allThreads.isEmpty {
                Spacer()
                if isLoading {
                    ProgressView().tint(RemoraTheme.accent).frame(maxWidth: .infinity)
                } else {
    ...
    private var newSessionButton: some View {
        Button {
            ...
            HStack {
                if isStartingNewSession {
                    ProgressView()
                        .controlSize(.small)
                        .tint(RemoraTheme.textOnAccent)
                } else {
                    Image(systemName: "plus")
                        .remoraFont(.subheadline, weight: .medium)
                    Text("New Session")
        ...
        .keyboardShortcut(RemoraPlatform.isCatalyst ? nil : KeyboardShortcut("n", modifiers: [.command]))
    ...
    private func runtimeKindPill(label: String, icon: String, kind: AgentRuntimeKind?) -> some View {
        ...
        return Button {
            ...
            .remoraFont(.caption)
            .foregroundColor(isActive ? RemoraTheme.textOnAccent : RemoraTheme.textSecondary)
            .padding(.horizontal, 10)
            .padding(.vertical, 6)
            .background(isActive ? RemoraTheme.accent : RemoraTheme.surface.opacity(0.65))
            .overlay(
                Capsule()
                    .stroke(isActive ? RemoraTheme.accent : RemoraTheme.border.opacity(0.7), lineWidth: 1)
            )
            .clipShape(Capsule())
        }
        .buttonStyle(.plain)
    ...
    private func workspaceGroupHeader(_ group: WorkspaceSessionGroup) -> some View {
        ...
        return Button {
            ...
            .overlay(alignment: .bottom) {
                Rectangle()
                    .fill(RemoraTheme.border.opacity(0.75))
                    .frame(height: 1)
            }
        }
        .buttonStyle(.plain)
            ...
            HStack(alignment: .top, spacing: 6) {
                ...
                HStack(alignment: .top, spacing: 6) {
                    if hasTurnActive {
                        ...
                        Circle().fill(RemoraTheme.textMuted.opacity(0.4)).frame(width: 8, height: 8).padding(.top, 3)
                    }
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        HStack(alignment: .firstTextBaseline, spacing: 6) {
                            ...
                            if thread.isSubagent {
                                ...
                                .padding(.vertical, 2)
                                .background(RemoraTheme.success)
                                .cornerRadius(4)
        ...
        .background {
            if isActive {
                RoundedRectangle(cornerRadius: 6)
                    .fill(RemoraTheme.surfaceLight.opacity(0.55))
            }
        }
        .contentShape(Rectangle())
        .hoverEffect(.highlight)
        ...
        return Group {
            if hasLineage {
                VStack(alignment: .leading, spacing: 5) {
                    Divider().background(RemoraTheme.border.opacity(0.7))
        ...
        Text("\(title) \(count)")
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 5)
                    .stroke(isInteractive ? RemoraTheme.accent.opacity(0.5) : RemoraTheme.border.opacity(0.5), lineWidth: 1)
            )
            .cornerRadius(5)
    }
    ...
    private func subagentStatusIndicator(_ status: AppSubagentStatus) -> some View {
        ...
        case .pendingInit, .running, .unknown:
            Circle()
                .fill(RemoraTheme.textMuted.opacity(0.4))
                .frame(width: 8, height: 8)
        }
    }
...
#Preview("Sessions Screen") {
    RemoraPreviewScene(
        appModel: RemoraPreviewData.makeSidebarAppModel(),
        appState: RemoraPreviewData.makeAppState()
    ) {
        NavigationStack {
