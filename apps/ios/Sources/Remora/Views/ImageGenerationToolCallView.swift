...
struct ImageGenerationToolCallView: View {
    ...
    private var imagePreview: some View {
        if let bytes = data.imagePNG {
            ...
            placeholderTile(icon: "exclamationmark.triangle.fill", message: "Image unavailable", tone: RemoraTheme.danger)
        } else {
            placeholderTile(icon: "photo", message: "Image unavailable", tone: RemoraTheme.textSecondary)
        }
    }
            ...
            Text(promptExpanded ? prompt : collapsedPreview(prompt))
                ...
                .background(
                    RoundedRectangle(cornerRadius: 8, style: .continuous)
                        .fill(RemoraTheme.codeBackground.opacity(0.82))
                )
        }
    }
    ...
    private func placeholderTile(icon: String, message: String, tone: Color) -> some View {
        VStack(spacing: 8) {
            Image(systemName: icon)
                .remoraFont(size: 24, weight: .medium)
...
private struct ImageGenerationLoadingTile: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .fill(RemoraTheme.accent.opacity(pulse ? 0.18 : 0.08))
                    .frame(width: 48, height: 48)
                    .overlay(
                        RoundedRectangle(cornerRadius: 12, style: .continuous)
                            .stroke(RemoraTheme.accent.opacity(0.32), lineWidth: 0.5)
                    )
            ...
            VStack(spacing: 5) {
                ...
                HStack(spacing: 5) {
                    ForEach(0..<3, id: \.self) { index in
                        Capsule()
                            .fill(RemoraTheme.accent.opacity(0.42))
