...
/// app launch in `RemoraApp` so any view can resolve an `AgentId` to
    ...
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: size * 0.2)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: size * 0.2)
                        .stroke(RemoraTheme.textPrimary.opacity(0.25), lineWidth: 0.5)
                )
            Text(monogramLetter)
        ...
        Text("BETA")
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 3)
                    .stroke(RemoraTheme.accent.opacity(0.6), lineWidth: 0.5)
            )
    }
}
