...
struct DiscoveryView: View {
        ...
        Button(action: action) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(alignment: .top, spacing: 14) {
                    Image(systemName: icon)
                        ...
                        .background(
                            Circle()
                                .fill(RemoraTheme.accent.opacity(isRecommended ? 0.16 : 0.10))
                        )
                        .padding(.top, 2)
                        ...
                        HStack(spacing: 8) {
                            ...
                            if let badge {
                                Text(badge)
                                    ...
                                    .background(
                                        Capsule()
                                            .fill(RemoraTheme.accent.opacity(0.14))
                                    )
                                    .overlay(
                                        Capsule()
                                            .stroke(RemoraTheme.accent.opacity(0.45), lineWidth: 0.6)
                                    )
                            }
                        }
                        Text(subtitle)
            ...
            .background(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .fill(RemoraTheme.surface.opacity(isRecommended ? 0.85 : 0.6))
            )
            .overlay(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .stroke(
                        RemoraTheme.accent.opacity(isRecommended ? 0.45 : 0.18),
        ...
        return Button {
            ...
            HStack(spacing: 12) {
                ...
                if let progressTag = progressTag(for: serverSnapshot) {
                    ...
                    ProgressView().controlSize(.small).tint(RemoraTheme.accent)
        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(environment.displayName)
                    .remoraFont(.subheadline)
                    .foregroundColor(environment.online ? RemoraTheme.textPrimary : RemoraTheme.textSecondary)
            ...
            statusTag(
                ...
                color: environment.online ? (environment.busy ? .orange : RemoraTheme.accent) : RemoraTheme.textMuted
            )
        }
        .padding(.vertical, 2)
    }
    ...
    private func progressTag(
        ...
        case .completed where step.kind == .connected:
            color = RemoraTheme.accentStrong
...
#Preview("Discovery") {
    RemoraPreviewScene(
        appModel: RemoraPreviewData.makeDiscoveryAppModel(),
        includeBackground: false
    ) {
        NavigationStack {
            DiscoveryView(
                autoStartDiscovery: false,
                initialServers: RemoraPreviewData.sampleDiscoveryServers
            )
        }
    }
}
