...
struct VoiceCallView: View {
    ...
    var body: some View {
        ZStack {
            RemoraTheme.backgroundGradient
                .ignoresSafeArea()
        ...
        VStack(spacing: 12) {
            ...
            HStack(spacing: 8) {
                Text(session.threadTitle)
                    .font(RemoraFont.styled(.caption, weight: .semibold))
            ...
            Button(role: .destructive) {
                ...
                HStack(spacing: 10) {
                    ...
                    Text(session.phase == .error ? "Close" : "Hang Up")
                        .font(RemoraFont.styled(.callout, weight: .semibold))
                }
                .foregroundColor(.white)
                .padding(.horizontal, 22)
                .padding(.vertical, 14)
                .background(Capsule().fill(RemoraTheme.danger))
            }
            .buttonStyle(.plain)
...
private struct CompactSpeakerIndicator: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 6) {
                Circle()
                    .fill(active ? tint : RemoraTheme.textMuted.opacity(0.45))
                    .frame(width: 7, height: 7)
                Text(title)
                    .font(RemoraFont.styled(.caption, weight: .semibold))
                    .foregroundColor(active ? RemoraTheme.textPrimary : RemoraTheme.textSecondary)
            }
                ...
                VStack(spacing: 0) {
                    VStack(spacing: 26) {
                        ...
                        if case .thinking = threadStatus {
                            Text("...")
                                .font(RemoraFont.monospaced(.title3, weight: .semibold))
                                .foregroundColor(RemoraTheme.textMuted)
                        }
                    }
    ...
    private var placeholder: some View {
        VStack(spacing: 10) {
            ...
            Text("The live conversation, tool calls, and other text output will appear here.")
                .font(RemoraFont.styled(.body, scale: textScale))
                .foregroundColor(RemoraTheme.textSecondary)
...
private struct VoiceCreditsEntryRow: View {
    ...
    private var bodyColor: Color {
        switch entry.kind {
        case .error:
            return RemoraTheme.danger.opacity(0.92)
        case .reasoning:
            return RemoraTheme.textSecondary
        default:
            return RemoraTheme.textPrimary
        }
    }
                    ...
                    Image(uiImage: uiImage)
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: 12, style: .continuous)
                                .stroke(RemoraTheme.border.opacity(0.4), lineWidth: 0.5)
                        )
        ...
        NavigationStack {
            List(session.debugEntries) { entry in
                Text("\(entry.timestamp.formatted(date: .omitted, time: .standard)) \(entry.line)")
                    .font(RemoraFont.monospaced(.caption2))
                    .foregroundColor(RemoraTheme.textPrimary)
                    .textSelection(.enabled)
