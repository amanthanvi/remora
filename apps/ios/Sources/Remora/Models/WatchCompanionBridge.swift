...
final class WatchCompanionBridge: NSObject {
    ...
    private static let appGroupSuite = "group.com.remora.app"
    ...
    private static let complicationKinds = [
        "RemoraCircularComplication",
        "RemoraCornerComplication",
        "RemoraRectangularComplication",
    ]
    ...
    static func makeApprovalNotificationRequest(
        ...
        return UNNotificationRequest(
            identifier: "remora.approval.\(approval.id)",
            content: content,
            trigger: nil
        )
    }
    ...
    /// Build a `RemoraComplicationPayload` Data slice per known server. The
    ...
    /// reconnects.
    func currentServerListPayload() -> RemoraServerListPayload {
        let servers = (AppModel.shared.snapshot?.servers ?? []).map {
            RemoraServerListPayload.Server(
                id: $0.serverId,
                displayName: $0.displayName
            )
        }
        return RemoraServerListPayload(servers: servers)
    }
    ...
    private func push(payload: WatchSnapshotPayload) {
        ...
        pushThrottle = Task { @MainActor [weak self, transport] in
            ...
            do {
                try transport.updateApplicationContext(["remora.snapshot": data])
