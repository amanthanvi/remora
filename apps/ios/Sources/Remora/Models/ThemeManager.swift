...
extension Notification.Name {
    static let themeDidChange = Notification.Name("com.remora.themeDidChange")
}

/// Thread-safe store for resolved themes, accessible from any isolation context.
/// ThemeManager writes here; RemoraTheme reads from here.
...
enum RemoraAppearanceMode: String, CaseIterable, Identifiable {
    case system
    case light
    case dark
...
final class ThemeManager {
    ...
    private(set) var appearanceMode: RemoraAppearanceMode = .system
    ...
    var selectedLightSlug: String {
        get { UserDefaults.standard.string(forKey: "selectedLightTheme") ?? "remora-light" }
        set { UserDefaults.standard.set(newValue, forKey: "selectedLightTheme") }
    }

    var selectedDarkSlug: String {
        get { UserDefaults.standard.string(forKey: "selectedDarkTheme") ?? "remora-dark" }
    ...
    func setAppearanceMode(_ mode: RemoraAppearanceMode) {
    ...
    private static func storedAppearanceMode() -> RemoraAppearanceMode {
        guard let raw = UserDefaults.standard.string(forKey: appearanceModeKey) else {
            return .system
        }
        return RemoraAppearanceMode(rawValue: raw) ?? .system
    }
    ...
    /// so auxiliary surfaces can read it.
