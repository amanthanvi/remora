...
/// (`_remora-pair._tcp.`) whose port is owned by the Rust pair host, then
...
final class MacPairingHost: NSObject {
    ...
    private static let pairServiceType = "_remora-pair._tcp."
    ...
    private static let macIdUserDefaultsKey = "remora.mac_pair_id"
    ...
    /// the iPhone at a working local Codex when `RemoraPlatform.isDirectDistMac`
    ...
    func startIfNeeded() {
        guard RemoraPlatform.isCatalyst else { return }
        guard !isBroadcasting else { return }
        isBroadcasting = true
        let macId = Self.resolveMacId()
        let deviceName = RemoraPlatform.localRuntimeDisplayName()
    ...
    private func startBLEAdvertiser(macId: String) {
        ...
        advertiser.start(localName: "remora:\(prefix)")
    ...
    private func showConfirmDialog(distanceM: Float?) {
        ...
        let alert = UIAlertController(
            ...
            message: "\(deviceLabel)\(distanceSuffix) wants to pair with this Mac as its Remora home base.",
