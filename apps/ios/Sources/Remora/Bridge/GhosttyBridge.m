...
static NSString *const RemoraGhosttyErrorDomain = @"com.remora.app.ghostty";

static void RemoraGhosttyResizeBackingLayers(UIView *view, CGFloat scale);
static void RemoraGhosttyWakeup(void *userdata);
static bool RemoraGhosttyAction(ghostty_app_t app, ghostty_target_s target, ghostty_action_s action);
static bool RemoraGhosttyReadClipboard(void *userdata, ghostty_clipboard_e clipboard, void *request);
static void RemoraGhosttyConfirmReadClipboard(void *userdata, const char *title, void *request, ghostty_clipboard_request_e requestType);
static void RemoraGhosttyWriteClipboard(void *userdata, ghostty_clipboard_e clipboard, const ghostty_clipboard_content_s *contents, size_t count, bool confirm);
static void RemoraGhosttyCloseSurface(void *userdata, bool processActive);
static void RemoraGhosttyExternalWrite(void *userdata, const uint8_t *data, uintptr_t length);
static dispatch_queue_t RemoraGhosttyDestroyQueue(void);

@interface RemoraGhosttyTerminal ()
@property (nonatomic, weak) UIView *view;
- (void)draw;
@end

@implementation RemoraGhosttyTerminal {
...
+ (BOOL)ensureInitialized:(NSError **)error {
    ...
    if (error != NULL) {
        *error = [NSError errorWithDomain:RemoraGhosttyErrorDomain
                                     code:initResult
...
- (nullable instancetype)initWithView:(UIView *)view error:(NSError **)error {
    ...
    if (![RemoraGhosttyTerminal ensureInitialized:error]) {
        return nil;
    }
    ...
    runtimeConfig.wakeup_cb = RemoraGhosttyWakeup;
    runtimeConfig.action_cb = RemoraGhosttyAction;
    runtimeConfig.read_clipboard_cb = RemoraGhosttyReadClipboard;
    runtimeConfig.confirm_read_clipboard_cb = RemoraGhosttyConfirmReadClipboard;
    runtimeConfig.write_clipboard_cb = RemoraGhosttyWriteClipboard;
    runtimeConfig.close_surface_cb = RemoraGhosttyCloseSurface;
    ...
    surfaceConfig.external_pty_write = RemoraGhosttyExternalWrite;
    ...
    RemoraGhosttyResizeBackingLayers(view, surfaceConfig.scale_factor);
...
- (void)invalidate {
    ...
    if (surface != NULL || app != NULL || config != NULL) {
        RemoraGhosttyTerminal *terminal = self;
        dispatch_async(RemoraGhosttyDestroyQueue(), ^{
            (void)terminal;
            if (surface != NULL) {
...
- (void)resizeToWidth:(CGFloat)width height:(CGFloat)height scale:(CGFloat)scale {
    ...
    RemoraGhosttyResizeBackingLayers(_view, resolvedScale);
...
- (RemoraGhosttySurfaceMetrics)surfaceMetrics {
    RemoraGhosttySurfaceMetrics out = {0};
    if (_surface == NULL) {
        return out;
    }
...
static ghostty_input_key_e RemoraGhosttyKeyToGhosttyKey(RemoraGhosttyKey key) {
    switch (key) {
        case RemoraGhosttyKeyEnter:      return GHOSTTY_KEY_ENTER;
        case RemoraGhosttyKeyTab:        return GHOSTTY_KEY_TAB;
        case RemoraGhosttyKeyBackspace:  return GHOSTTY_KEY_BACKSPACE;
        case RemoraGhosttyKeyEscape:     return GHOSTTY_KEY_ESCAPE;
        case RemoraGhosttyKeySpace:      return GHOSTTY_KEY_SPACE;
        case RemoraGhosttyKeyArrowUp:    return GHOSTTY_KEY_ARROW_UP;
        case RemoraGhosttyKeyArrowDown:  return GHOSTTY_KEY_ARROW_DOWN;
        case RemoraGhosttyKeyArrowLeft:  return GHOSTTY_KEY_ARROW_LEFT;
        case RemoraGhosttyKeyArrowRight: return GHOSTTY_KEY_ARROW_RIGHT;
        case RemoraGhosttyKeyPageUp:     return GHOSTTY_KEY_PAGE_UP;
        case RemoraGhosttyKeyPageDown:   return GHOSTTY_KEY_PAGE_DOWN;
        case RemoraGhosttyKeyHome:       return GHOSTTY_KEY_HOME;
        case RemoraGhosttyKeyEnd:        return GHOSTTY_KEY_END;
        case RemoraGhosttyKeyDelete:     return GHOSTTY_KEY_DELETE;
        case RemoraGhosttyKeyInsert:     return GHOSTTY_KEY_INSERT;
        case RemoraGhosttyKeyUnidentified:
        default:                         return GHOSTTY_KEY_UNIDENTIFIED;
    }
}

- (BOOL)dispatchKeyAction:(int)action
                      key:(RemoraGhosttyKey)key
                     mods:(int)mods
                     text:(NSString *)text
    ...
    event.keycode = (uint32_t)RemoraGhosttyKeyToGhosttyKey(key);
...
static dispatch_queue_t RemoraGhosttyDestroyQueue(void) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        queue = dispatch_queue_create("com.remora.app.ghostty.destroy", attr);
    });
    return queue;
}

static RemoraGhosttyTerminal *RemoraGhosttyTerminalFromUserdata(void *userdata) {
    if (userdata == NULL) {
        return nil;
    }
    return (__bridge RemoraGhosttyTerminal *)userdata;
}

static void RemoraGhosttyWakeup(void *userdata) {
    RemoraGhosttyTerminal *terminal = RemoraGhosttyTerminalFromUserdata(userdata);
    if (terminal == nil) {
        return;
    }
