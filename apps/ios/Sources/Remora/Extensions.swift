...
extension View {
    func remoraFont(size: CGFloat, weight: Font.Weight = .regular) -> some View {
        modifier(ScaledSizeFontModifier(size: size, weight: weight))
    }

    func remoraFont(_ style: Font.TextStyle, weight: Font.Weight = .regular) -> some View {
        modifier(ScaledStyleFontModifier(style: style, weight: weight))
    }

    func remoraMonoFont(size: CGFloat, weight: Font.Weight = .regular) -> some View {
...
private struct ScaledStyleFontModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content.font(RemoraFont.styled(style, weight: weight, scale: textScale))
    }
}
            ...
            content
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: cornerRadius)
                        .stroke((tint ?? RemoraTheme.border).opacity(0.4), lineWidth: 1)
                )
        }
    }
}
...
struct GlassRoundedRectModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        if #available(iOS 26.0, *) {
            ...
            content
                .background(RemoraTheme.surfaceLight)
