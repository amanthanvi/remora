...
struct RemoraServerListPayload: Codable, Equatable {
...
enum RemoraServerListStore {
    static let appGroup = "group.com.remora.app"
    static let key = "servers.v1"

    static func current() -> RemoraServerListPayload? {
        guard
            let defaults = UserDefaults(suiteName: appGroup),
            let data = defaults.data(forKey: key)
        else { return nil }
        return try? JSONDecoder().decode(RemoraServerListPayload.self, from: data)
    }

    static func write(_ payload: RemoraServerListPayload) {
...
///
/// Shape matches the per-server `RemoraComplicationEntry.Payload` written
/// in the aggregate path, so the same decoder can rehydrate either side.
enum RemoraPerServerComplicationStore {
    static let appGroup = "group.com.remora.app"
