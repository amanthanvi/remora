...
/// payload (Task #7). The shared types live in the RemoraWatchComplications
/// target and are also compiled into Remora via `project.yml` so the bridge
...
final class PerServerComplicationTests: XCTestCase {
    ...
    func testServerListPayloadRoundTripsThroughJSON() throws {
        let payload = RemoraServerListPayload(servers: [
            .init(id: "macbook", displayName: "MacBook Pro"),
            .init(id: "studio", displayName: "Mac Studio"),
        ])

        let data = try JSONEncoder().encode(payload)
        let decoded = try JSONDecoder().decode(RemoraServerListPayload.self, from: data)
    ...
    func testPerServerSnapshotsFilterRunningTaskToOwningServer() throws {
        ...
        let macbook = try JSONDecoder().decode(RemoraComplicationPayload.self, from: macbookData)
        ...
        let studio = try JSONDecoder().decode(RemoraComplicationPayload.self, from: studioData)
    ...
    func testPerServerSnapshotsEmitOfflineWhenWatchUnreachable() throws {
        ...
        let decoded = try JSONDecoder().decode(RemoraComplicationPayload.self, from: data)
