...
internal fun SyntaxHighlightedDiffBlock(
    ...
    val resolvedFontPx: Float = if (fontSize.isUnspecified) {
        RemoraTextStyle.caption * textScale
    } else {
        fontSize.value * textScale
    }
    val palette = remember(
        RemoraTheme.textBody.toArgb(),
        RemoraTheme.textSecondary.toArgb(),
        RemoraTheme.success.toArgb(),
        RemoraTheme.danger.toArgb(),
        RemoraTheme.accentStrong.toArgb(),
        RemoraTheme.codeBackground.toArgb(),
        RemoraTheme.surface.copy(alpha = 0.72f).toArgb(),
    ) {
        DiffSyntaxPalette(
            context = RemoraTheme.textBody.toArgb(),
            metadata = RemoraTheme.textSecondary.toArgb(),
            addition = RemoraTheme.success.toArgb(),
            deletion = RemoraTheme.danger.toArgb(),
            hunk = RemoraTheme.accentStrong.toArgb(),
            contextBackground = RemoraTheme.codeBackground.toArgb(),
            metadataBackground = RemoraTheme.surface.copy(alpha = 0.72f).toArgb(),
            additionBackground = RemoraTheme.success.copy(alpha = 0.12f).toArgb(),
            deletionBackground = RemoraTheme.danger.copy(alpha = 0.12f).toArgb(),
            hunkBackground = RemoraTheme.accentStrong.copy(alpha = 0.12f).toArgb(),
        )
    }
    ...
    AndroidView(
        ...
        update = { scrollView ->
            ...
            textView.setTextColor(RemoraTheme.textBody.toArgb())
