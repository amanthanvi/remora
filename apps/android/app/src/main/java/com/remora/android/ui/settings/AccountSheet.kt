    ...
    Column(
        ...
        when (account) {
            is Account.Chatgpt -> {
                Column(
                    ...
                    Text("Logged in", color = RemoraTheme.accent, fontSize = 13.sp)
                    Text(account.email, color = RemoraTheme.textPrimary, fontSize = 14.sp)
                }
            }
            ...
            is Account.ApiKey -> {
                Column(
                    ...
                    Text("API key configured", color = RemoraTheme.accent, fontSize = 13.sp)
                }
            }
        ...
        if (allowsLocalEnvApiKey) {
            if (hasStoredApiKey) {
                ...
                Text("Or save an API key for the local environment:", color = RemoraTheme.textSecondary, fontSize = 12.sp)
            }
        ...
        error?.let {
            Text(it, color = RemoraTheme.danger, fontSize = 12.sp)
        }
    }
}
