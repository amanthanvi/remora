...
object RemoraTheme {
    ...
    val isDark: Boolean
        get() = activeTheme.type == RemoraColorThemeType.DARK
    ...
    val monoFont: FontFamily
        get() = if (RemoraThemeManager.monoFontEnabled) BerkeleyMono else FontFamily.Monospace
    ...
    val backgroundBrush: Brush
        get() =
            ...
            colors =
                listOf(
                    background,
                    RemoraResolvedTheme.adjustBrightness(
                        background,
...
private val RemoraPlatformTextStyle = PlatformTextStyle(includeFontPadding = false)

private fun remoraTextStyle(
    ...
    fontWeight = fontWeight,
    fontSize = fontSize,
    platformStyle = RemoraPlatformTextStyle,
)
...
@Composable
fun RemoraAppTheme(content: @Composable () -> Unit) {
    val appContext = LocalContext.current.applicationContext
    DisposableEffect(appContext) {
        RemoraThemeManager.initialize(appContext)
        onDispose {}
    }

    val systemIsDark = isSystemInDarkTheme()
    val appearanceMode = RemoraThemeManager.appearanceMode
    val lightThemeSlug = RemoraThemeManager.lightTheme.slug
    val darkThemeSlug = RemoraThemeManager.darkTheme.slug
    LaunchedEffect(appearanceMode, systemIsDark, lightThemeSlug, darkThemeSlug) {
        RemoraThemeManager.applySystemTheme(systemIsDark)
    }

    val activeTheme = RemoraThemeManager.activeTheme
    RemoraSystemBarsEffect(useDarkTheme = activeTheme.type == RemoraColorThemeType.DARK)

    val colorScheme =
        remember(activeTheme.slug, activeTheme.type) {
            if (activeTheme.type == RemoraColorThemeType.DARK) {
                darkColorScheme(
    ...
    val monoFontEnabled = RemoraThemeManager.monoFontEnabled
...
@Composable
private fun RemoraSystemBarsEffect(useDarkTheme: Boolean) {
...
@Composable
private fun RemoraThemePreview() {
    RemoraAppTheme {
        Surface(color = RemoraTheme.background) {
            Text(
                text = "Remora Theme",
