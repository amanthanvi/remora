 ...
 * shell calls `window.__RemoraAppBridge.saveAppState(payloadJson, schema)`
 * whenever user state needs to be persisted; this forwards that into the
 * [SavedAppsStore] debouncer. `window.__RemoraAppBridge.structuredResponse(...)`
 ...
 *
 * The name `__RemoraAppBridge` is the single symbol the JS shell references on
...
class SavedAppBridge(
    ...
    companion object {
        ...
        const val INTERFACE_NAME = "__RemoraAppBridge"
    }
}
