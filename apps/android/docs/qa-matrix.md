...
| Auto-upsert                               | When the model finalizes a `show_widget` with `app_id = "fitness-tracker"` on a local-server thread, the Rust hook calls `saved_app_upsert(directory, originThreadId, appId, title, html, w, h, schema)` and writes to `{filesDir}/RemoraPreferences/apps/saved_apps.json` + `html/<uuid>.html`. No Kotlin-initiated promote call is needed.                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Home rows (no takeover)                   | There is no home-row takeover. `HomeDashboardScreen` renders `SessionCanvasRow` for every recent session, including sessions that have saved apps originating from their thread — a saved app never replaces its session's row. Saved apps are reached from the Saved-as chip, the Apps list, or a `Route.SavedApp` deep link. Swipe-to-hide on the session works as normal.                                                                                                                                                                                                                                                                                                                                                           |
...
| Detail relaunch                           | Tapping a row (or a Saved-as chip) pushes `Route.SavedApp(uuid)`. `SavedAppScreen` calls `savedAppGet(dir, uuid)` on enter, hydrates the WebView with `wrapWidgetHtml(html, AppStateInjection(stateJson, schemaVersion))`, and registers `__RemoraAppBridge` via `addJavascriptInterface`.                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Same slug in two threads                  | Model emitting `app_id = "fitness-tracker"` in two different origin threads creates two independent saved apps (distinct UUIDs, separate state files). The Apps list shows both, and each thread's Saved-as chip resolves to its own app.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Bridge                                | JS posts `{_type, ...}` messages through `__postWidgetMessage`, which routes: `saveAppState` → `__RemoraAppBridge`, `height`/`sendPrompt`/`openLink`/`ready` → `__RemoraWidgetBridge`, with the iOS `webkit.messageHandlers.widget` fallback (no-op on Android). Both bridges coexist on the saved-app WebView.                                                                                          |
