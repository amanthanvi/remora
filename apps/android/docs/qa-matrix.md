...
| Global Sessions inbox                   | Mission Control links to a matching flat inbox on both platforms. Rust applies bounded encrypted search plus Host/project/runtime/status/attention/time filters before 50-row cursor paging and at most 200 decrypt candidates. Per-Host live/cached/unavailable freshness and partial-result guidance match across platforms. Cached rows are visibly stale and non-actionable; live rows open directly. Scrolling/filtering never hydrates a timeline. |
| Needs You lifecycle                     | Live completion/failure events enter Needs You without flooding historical idle Threads. Opening acknowledges terminal attention. Session actions expose terminal-only acknowledgement, one-hour snooze, and confirmed Host archive; blocking approvals/questions remain actionable in-app. State and policy come from the Rust device database/projection. |
